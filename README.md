...
> 默认配置指向作者的演示服务器，由于可能随时重启或下线，因此正式使用建议按本文档自建服务端。（出于安全性考虑，现在已将默认配置替换为内网ip127.0.0.0/0）
...
```bash
git clone https://github.com/Yzk258/QuickClipboard.git
cd QuickClipboard
...
**内置流量防护**：服务端针对公网暴露场景做了基础防护，默认开启——全局/单 IP 并发连接上限、单 IP 新建连接频率限制、单连接消息频率限制、登录失败封禁（默认窗口内失败 5 次封禁 10 分钟），以及握手 / 空闲 / 连接最长存活三类超时。参数可在 `src/server.py` 顶部的「基础流量防护」配置区调整；若需对抗大规模分布式 DDos，仍建议叠加云厂商安全组 / 高防等外部设施。
